...
Redaction is user-driven and manual. BugDrop does not detect or mask sensitive content automatically, so users should review manual screenshots before submitting; automatic screenshots (`data-screenshot="auto"`) upload without a review step.
...
Redaction is manual. In the screenshot review step, the **Redact** tool paints
opaque black rectangles into the PNG before upload, and **Undo** removes the
latest one. The submitted image contains the rectangles; the page DOM is not
mutated.

BugDrop does not mask elements automatically. Earlier versions covered elements
marked with `data-bugdrop-mask`, `data-bugdrop-redact`, `data-bd-redact` or
`data-bugdrop-redacted`, plus password and credit-card inputs. Those attributes
are now ignored: hosts that marked whole views ended up with screenshots that
were almost entirely black, so the decision of what to cover belongs to the
person submitting the report.

Automatic screenshot mode (`data-screenshot="auto"`) uploads without a review
step. Prefer `optional` or `required` on pages that render sensitive data, or
disable screenshots there.
