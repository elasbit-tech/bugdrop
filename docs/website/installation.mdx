...
BugDrop does not mask page content automatically. In the manual screenshot flows
(`data-screenshot="optional"` or `"required"`), the review step offers a **Redact**
tool so the reporter can black out sensitive regions before submitting. Automatic
screenshots (`data-screenshot="auto"`) upload without that step, so avoid auto mode
on pages that render customer data. See
[Screenshot redaction](/docs/security#screenshot-redaction) on the Security page for details.
