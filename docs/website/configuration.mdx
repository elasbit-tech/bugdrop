...
Redaction is manual and controlled by the person submitting feedback: the screenshot review step has a **Redact** tool that paints opaque black rectangles into the uploaded image, and **Undo** removes them. BugDrop does not mask any element automatically; the `data-bugdrop-mask` family of attributes is ignored. Auto mode warns users that a full-page screenshot will be attached, but it does not show the manual picker or annotation review step.
